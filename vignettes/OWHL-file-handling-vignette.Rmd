...
pressure offset due to factors such as manufacturing variance in the pressure sensor chip or 
...
readings from the data set while the OWHL was at the sea surface just prior to deployment underwater. Ideally the value recorded by the OWHL at the ocean surface would be equivalent to the local atmospheric pressure, but any difference that exists can be removed from the entire timeseries. 
...
included the data from the surface period prior to the deployment, which we will remove at a later step. 
...
lines(x = wavesZC$DateTime, y = wavesZC$Hsig, type = 'b',
      col = 'red')
legend('topleft',legend=c('Hm0, spectral method','Hsig, zero-cross'), col = c('black','red'), lty = c(1,1),
       pch = c(1,1))
par(op)
```
...
legend('topleft',legend=c('Peak period, spectral method',
                          'Period of upper 1/3 of waves, zero-cross'), col = c('black','red'), lty = c(1,1),
       pch = c(1,1))
par(op)
```
